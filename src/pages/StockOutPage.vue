<template>
  <q-page class="q-pa-md">
    <div class="row q-mb-md">
      <div class="col">
        <h5 class="q-mt-none q-mb-md">出库登记</h5>
      </div>
      <div class="col-auto">
        <q-btn color="secondary" icon="arrow_back" label="返回" :to="'/inventory'" />
      </div>
    </div>

    <q-card>
      <q-card-section>
        <q-form @submit="onSubmit" class="q-gutter-md">
          <div class="row q-col-gutter-md">
            <!-- 商品选择 -->
            <div class="col-12">
              <q-select
                v-model="selectedProduct"
                :options="productOptions"
                label="选择商品 *"
                outlined
                use-input
                hide-selected
                fill-input
                input-debounce="0"
                emit-value
                map-options
                :rules="[val => !!val || '请选择一个商品']"
                option-label="label"
                option-value="value"
                @filter="filterProducts"
              />
            </div>

            <!-- 数量和单位 -->
            <div class="col-12 col-md-6">
              <q-input
                v-model.number="quantity"
                label="出库数量 *"
                type="number"
                outlined
                :rules="[
                  val => (val !== null && val !== undefined) || '请输入出库数量',
                  val => val > 0 || '数量必须大于0',
                  val => val <= currentStock || '出库数量不能超过当前库存'
                ]"
              />
            </div>

            <div class="col-12 col-md-6">
              <q-input
                v-model="unit"
                label="单位"
                outlined
                readonly
                :disable="!selectedProduct"
              />
              <div class="text-caption q-mt-xs" v-if="selectedProduct">
                当前库存: {{ currentStock }}
              </div>
            </div>

            <!-- 出库信息 -->
            <div class="col-12 col-md-6">
              <q-input
                v-model="reference"
                label="参考单号（如订单号）"
                outlined
              />
            </div>

            <div class="col-12 col-md-6">
              <q-select
                v-model="outReason"
                :options="reasonOptions"
                label="出库原因"
                outlined
                emit-value
                map-options
              />
            </div>

            <!-- 备注 -->
            <div class="col-12">
              <q-input
                v-model="notes"
                label="备注"
                type="textarea"
                outlined
                autogrow
              />
            </div>

            <!-- 提交按钮 -->
            <div class="col-12 q-gutter-md q-mt-md">
              <q-btn type="submit" color="primary" label="确认出库" :loading="saving" />
              <q-btn label="取消" flat color="secondary" :to="'/inventory'" />
            </div>
          </div>
        </q-form>
      </q-card-section>
    </q-card>
  </q-page>
</template>

<script setup lang="ts">
import { ref, computed, watch, onMounted } from 'vue';
import { useRoute, useRouter } from 'vue-router';
import { useQuasar } from 'quasar';
import { useProductStore } from '@/stores/product-store';
import { useInventoryStore } from '@/stores/inventory-store';

const $q = useQuasar();
const route = useRoute();
const router = useRouter();
const productStore = useProductStore();
const inventoryStore = useInventoryStore();

// 状态
const saving = ref(false);
const selectedProduct = ref(null);
const quantity = ref(1);
const unit = ref('');
const reference = ref('');
const outReason = ref('销售');
const notes = ref('');
const currentStock = ref(0);

// 产品列表
const productOptions = ref([]);
const originalProducts = ref([]);

// 出库原因选项
const reasonOptions = [
  { label: '销售', value: '销售' },
  { label: '损坏', value: '损坏' },
  { label: '过期', value: '过期' },
  { label: '转移', value: '转移' },
  { label: '其他', value: '其他' }
];

// 监听选择的商品变化
watch(selectedProduct, async (newProductId) => {
  if (newProductId) {
    const product = productStore.getProductById(newProductId);
    if (!product) return;
    
    // 获取库存信息
    const inventory = inventoryStore.getInventoryByProductId(newProductId);
    if (inventory) {
      unit.value = inventory.unit || '个';
      currentStock.value = inventory.quantity;
      
      // 如果当前库存小于设定的出库数量，调整出库数量
      if (quantity.value > currentStock.value) {
        quantity.value = currentStock.value;
      }
    } else {
      unit.value = '个';
      currentStock.value = 0;
      quantity.value = 0;
    }
  } else {
    resetForm();
  }
});

// 根据查询条件筛选商品
const filterProducts = (val, update) => {
  if (val === '') {
    update(() => {
      productOptions.value = originalProducts.value;
    });
    return;
  }

  update(() => {
    const needle = val.toLowerCase();
    productOptions.value = originalProducts.value.filter(
      v => v.label.toLowerCase().indexOf(needle) > -1 ||
           v.sku.toLowerCase().indexOf(needle) > -1
    );
  });
};

// 重置表单
const resetForm = () => {
  selectedProduct.value = null;
  quantity.value = 1;
  unit.value = '';
  reference.value = '';
  outReason.value = '销售';
  notes.value = '';
  currentStock.value = 0;
};

// 提交表单
const onSubmit = async () => {
  // 验证库存是否足够
  if (quantity.value > currentStock.value) {
    $q.notify({
      color: 'negative',
      message: '出库数量不能超过当前库存',
      icon: 'warning'
    });
    return;
  }
  
  saving.value = true;
  
  try {
    // 记录出库事务
    await inventoryStore.recordTransaction({
      product_id: selectedProduct.value,
      type: 'out',
      quantity: quantity.value,
      notes: `${outReason.value}: ${notes.value || '无备注'}, 参考号: ${reference.value || '无'}`,
      reference: reference.value,
      operator: 'admin'
    });
    
    $q.notify({
      color: 'positive',
      message: `出库成功! ${quantity.value} ${unit.value} 已从库存移除`,
      icon: 'check'
    });
    
    // 重置表单或返回库存页面
    if (route.query.returnTo === 'stay') {
      resetForm();
    } else {
      router.push('/inventory');
    }
  } catch (error) {
    $q.notify({
      color: 'negative',
      message: `出库失败: ${(error as Error).message}`,
      icon: 'warning'
    });
  } finally {
    saving.value = false;
  }
};

// 初始化产品选项
const initProductOptions = () => {
  // 只显示有库存的产品
  const productsWithStock = productStore.products.filter(p => {
    const inventory = inventoryStore.getInventoryByProductId(p.id as string);
    return inventory && inventory.quantity > 0;
  });
  
  originalProducts.value = productsWithStock.map(p => {
    const inventory = inventoryStore.getInventoryByProductId(p.id as string);
    return {
      label: `${p.name} (库存: ${inventory ? inventory.quantity : 0})`,
      value: p.id,
      sku: p.sku
    };
  });
  
  productOptions.value = [...originalProducts.value];
};

// 页面初始化
onMounted(async () => {
  // 加载产品数据
  if (productStore.products.length === 0) {
    await productStore.fetchAllProducts();
  }
  
  // 加载库存数据
  if (inventoryStore.inventory.length === 0) {
    await inventoryStore.fetchAllInventory();
  }
  
  // 初始化产品选项
  initProductOptions();
  
  // 如果URL中有product参数，自动选择该产品
  const productId = route.query.product;
  if (productId) {
    selectedProduct.value = productId.toString();
  }
});
</script>